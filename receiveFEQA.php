...
function installFEQA ()
{
	chdir('./');
	$send_file = shell_exec('./installQA.sh'); 
	
}

function installPrev(){
	chdir('./');
	$send_file = shell_exec('./installPrev.sh');
}
...
$channel_send->queue_declare('feprod_receive', false, true, false, false);
...
$callback = function($req) {
	...
	$channel_send->basic_publish($msg, '', 'feprod_receive');
...
$channel_rec->basic_consume('feprod_send', '', false, false, false, false, $callback);
